	...
	public class FilterService : IEarlyBlocker, IMService {
		...
		public async Task<bool> FilterWords(IGuild guild, IUserMessage usrMsg, bool realExecution = true) {
			...
			if(filteredChannelWords.Count == 0 && filteredServerWords.Count == 0)
				return false;
			if(!wordsInMessage.Any(word => filteredChannelWords.Contains(word) || filteredServerWords.Contains(word)))
				return false;
			if(!realExecution)
				return true;
			try {
		...
		public async Task<bool> FilterInvites(IGuild guild, IUserMessage usrMsg, bool realExecution = true) {
			...
			if(!InviteFilteringChannels.Contains(usrMsg.Channel.Id) && !InviteFilteringServers.Contains(guild.Id) || !usrMsg.Content.IsDiscordInvite())
				return false;
			if(!realExecution)
				return true;
			try {
		...
		public async Task<bool> FilterZalgo(IGuild guild, IUserMessage usrMsg, bool realExecution = true) {
			...
			if(!ZalgoFilteringServers.Contains(guild.Id) && !ZalgoFilteringChannels.Contains(usrMsg.Channel.Id) || !IsZalgo(usrMsg.Content))
				return false;
			if(!realExecution)
				return true;
			try {
		...
		public bool IsZalgo(string s) {
			...
			if(floor == ceiling)
				percentile = scores[floor];
			else
				percentile = scores[floor] * (ceiling - k) + scores[ceiling] * (k - floor);
