...
namespace Mitternacht.Modules.Administration.Services {
	public class LogCommandService : IMService {
		...
		private readonly GuildTimezoneService _tzs;
		...
		private string PrettyCurrentTime(IGuild guild)
			=> $"【{CurrentTime(guild)}】";

		private string CurrentTime(IGuild guild)
			=> $"{(guild != null ? TimeZoneInfo.ConvertTime(DateTime.UtcNow, _tzs.GetTimeZoneOrUtc(guild.Id)) : DateTime.UtcNow):HH:mm:ss}";
		...
		private Task VerificationService_VerificationStep(VerificationProcess vp, VerificationStep step) {
			var _ = Task.Run(async () => {
				try {
					...
					if(TryGetLogSettingForGuild(guild.Id, out var logSetting) && logSetting.VerificationSteps != null){
		...
		private Task VerificationService_VerificationMessage(VerificationProcess vp, SocketMessage msg) {
			var _ = Task.Run(async () => {
				try {
					...
					if(TryGetLogSettingForGuild(guild.Id, out var logSetting) && logSetting.VerificationMessages != null){
		...
		private Task _client_UserUpdated(SocketUser before, SocketUser uAfter) {
			var _ = Task.Run(async () =>
			{
				try
				{
					...
					if (!TryGetLogSettingForGuild(g.Id, out var logSetting)
                    ...
                    //    if (!TryGetLogSettingForGuild(g.Id, out logSetting)
                    //        || (logSetting.UserUpdatedId == null))
                    //        return;
		...
		public Task TriggeredAntiProtection(PunishmentAction action, ProtectionType protection, params IGuildUser[] users) {
			var _ = Task.Run(async () =>
			{
				try
				{
					...
					if (!TryGetLogSettingForGuild(users.First().Guild.Id, out var logSetting)
		...
		private Task _client_ChannelDestroyed(IChannel ich) {
			var _ = Task.Run(async () =>
			{
				try
				{
					...
					if (!TryGetLogSettingForGuild(ch.Guild.Id, out var logSetting)
		...
		//            if (!TryGetLogSettingForGuild(guild.Id, out LogSetting logSetting)
		...
		private void UnsetLogSetting(ulong guildId, LogType logChannelType) {
			using var uow = _db.UnitOfWork;
			var logSetting = uow.GuildConfigs.For(guildId, set => set.Include(x => x.LogSetting)).LogSetting;
			...
			switch(logChannelType) {
				...
				case LogType.VoicePresenceTTS:
					...
					break;
				case LogType.VerificationSteps:
					logSetting.VerificationSteps = null;
					break;
				case LogType.VerificationMessages:
					logSetting.VerificationMessages = null;
					break;
			}
